...
export const initialContextMap = new WeakMap<Target, any>()
export const contextListMap = new WeakMap<Target, KeyToContextList>()
...
export function readonly<T extends object>(
  ...
  if (isRef(target)) {
    return isReadonly(target) ? target : (target as Ref).toReadonly()
  }
  return createReactiveObject(
    target,
    true,
...
type Listener = (value?: any, oldValue?: any) => void;
const listenerMap = new WeakMap<Target, Map<any, Listener[]>>();

export function subscribe(target: Target, key: any, listener: Listener) {
  ...
  return () => unsubcribe(target, key, listener);
}
...
function createReactiveObject(
  ...
  initialContextMap.set(proxy, context);
  return proxy
}
